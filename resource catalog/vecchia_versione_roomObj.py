...
class ResourcesServerREST(object):
	...
	def PUT(self,*uri):
		...
		if command=='insertPlatform':
			...
			platformFlag,newRoom=self.serverCatalog.insertRoom(platform_ID,room_ID,MRT,devices)
			...
			endPoints=json_body['endPoints']
			availableResources=json_body['availableResources']
			platformFlag, roomFlag, newDevice=self.serverCatalog.insertDevice(platform_ID,room_ID,device_ID,endPoints,availableResources)
			if platformFlag is False:
				raise cherrypy.HTTPError(404,"Platform Not found")
