...
class GrafanaCatalog():
    ...
    def getDashboardURL(self, platformID, roomID):
        ...
        self.url="http://"+self.server_url+"/api/dashboards/uid/"+self.uid
        r=requests.get(url=self.url, headers=self.headers, verify=False)
        self.data=r.json()
        #print(data)
        self.dash_url="http://"+self.server_url+self.data["meta"]["url"]+"?orgId="+self.orgID
    ...
    def createDatasource(self, platformID):
        ...
        self.url="http://"+self.server_url+'/api/datasources'
