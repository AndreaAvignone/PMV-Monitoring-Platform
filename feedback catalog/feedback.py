...
class feedbackREST():
	...
	def GET(self,*uri):
        ...
        if uriLen!=0:
            info=uri[0]
            platform= self.feedbackCatalog.retrievePlatform(info)
    ...
    def PUT(self, *uri):
        ...
        command=str(uri[0])
        elif command=='newFeedback':
            platform_ID=uri[1]
            room_ID=uri[2]
            value=json_body['feedback']
            newSetting=self.feedbackCatalog.setParameter(platform_ID,room_ID,value)
            if newSetting==True:
...
if __name__ == '__main__':
    ...
    cherrypy.tree.mount(feedbackCatalog, feedback.service, conf)
